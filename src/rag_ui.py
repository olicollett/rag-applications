...
ICON_DIR = './app_assets/'
reader_model_name = mini
collection_name = None # Defined in app
data_path = '../data/huberman_labs.json'
embedding_model_path = '/workspaces/rag-applications/notebooks/models/content/models/allminilm-finetuned-256'
###################################
...
# I don't have all of these available
...
def main(retriever: WeaviateWCS):
    ...
    with st.sidebar:
        ...
        alpha_input = st.slider('Alpha', min_value=0.0,max_value=1.0,value=0.5, step=0.1)
        retrieval_limit = st.slider('Retrieval Limit', min_value=1, max_value=200, value=10, step=1)
        reranker_topk = st.slider('Reranker TopK', min_value=1,max_value=10, value=2, step=1)
        temperature_input = st.slider('Temperature', min_value=0.0,max_value=1.0, value=0.2, step=0.01)
    ...
    # ##################
            ...
            for i, hit in enumerate(valid_response):
               ...
               title = hit['title']
               #show_length = hit['length_seconds'] # This didn't seem to work?
               ...
               with col2:
                   ...
                   st.image(image, caption=title.split('|')[0], width=200, use_container_width=False) # Updated this to avoid warning message for deprecated var
